...
const Index = () => {
  ...
  const HandelSearch = async () => {
    const res = await axios.get(
      `https://restcountries.com/v3.1/name/${sessionStorage.getItem("name")}`
    );
    setRes(res.data);
    return res;
  };
  ...
  const HandelReg = async () => {
    if (
      ...
      sessionStorage.getItem("reg") !== "All"
    ) {
      const res = await axios.get(
        `https://restcountries.com/v3.1/region/${sessionStorage.getItem("reg")}`
      );
      setRes(res.data);
      return res;
  ...
  const {
    isLoading: isSeraching,
    refetch: fechSerach,
    isError,
        } = useQuery("search", HandelSearch, { enabled: false });

  const {
    isLoading: loddingRegion,
    isError: errorLodingRegion,
    error,
    refetch: fechRegion,
  } = useQuery("region-data", HandelReg, { enabled: false });
  ...
  if (isError) {
    return (
      <h1 className=" text-blue-700 text-[25px]">{`No country named :${name}`}</h1>
    );
  }
  if (errorLodingRegion) {
    return (
      <h1 className=" text-blue-700 text-[25px]">{`error loding region ${error}`}</h1>
    );
  }
      ...
      <header className="flex  gap-2  fixed top-0  w-full z-20  justify-between p-[5px] sm:px-[50px] items-center bg-[#010508] h-28  border-b-[0.2px] border-[#30363D]">
        <div>
          ...
          <button
            ...
            className=" block sm:hidden border-[1px] border-[#30363D] rounded-lg p-1"
          >
